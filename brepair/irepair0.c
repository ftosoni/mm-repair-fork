...
/*  
 * Executes the RePair algorithm for integers, but never 
...
void prepare0 (relong len)
  ...
  { if (text[i] > alph) alph = text[i];
    ...
    // !!!!!!!!!!!!!!!!!!!! //
    fprintf(stderr, "Sorry very large texts not supported yet. Max: %ld MBs",MB/(3*sizeof(relong)));
    exit(2);
    // !!!!!!!!!!!!!!!!!!! // 
    did0 = 1;
    for (i=0;i<len-1;i++) 
...
// do a quadratic update of pairs until length*24 << Available MB
...
relong repair (FILE *R) { 
    ...
    while (n+1 > 0) {  // I guess this is equiavalent while(True)
      ...
      // take care of all occurrences of C[cpos]C[pos+1]  
      while (cpos != -1)
        ...
        // remove bc from L
           ...
           if (ssgte != u) {// there is ssgte
              ...
              if(forbidden_pair(pair.left,pair.right)) {
                ...
                L[cpos].prev = -id-1; // prev of old cpos points here
                rec->cpos = cpos;     // first cpos of this record is this 
              }
           }
